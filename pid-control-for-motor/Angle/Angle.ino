...
int incomingData; // This while convert input string into integer
 ...
 while (Serial.available()) { //Check if the serial data is available.
  ...
  if (readString.length() >0) { //Verify that the variable contains information
   ...
   Serial.println(readString.toInt());  //printing the input data in integer form
      incomingData = readString.toInt();   // here input data is store in integer form
Serial.write(incomingData);
}
REV = map (incomingData, 0, 360, 0, 4000); // mapping degree into pulse
